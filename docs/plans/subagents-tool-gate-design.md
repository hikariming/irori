# 设计：接入 pi-subagents 并让子代理复用 Irori 审核围栏
...
```
runIroriPiPrompt (prompt-runner.mjs)
  ...
  └─ createIroriPiSession (pi-session-adapter.mjs)
...
- **扩展装配层**：新建一个**独立、可被路径加载**的 pi 扩展
  `apps/desktop/sidecar/src/extensions/irori-tool-gate.mjs`，它：
...
runIroriPiPrompt
  ...
  └─ createIroriPiSession(... gateConfigPath)
```
...
```
       ...
       │  irori-tool-gate      │                      │  irori-tool-gate（继承）   │
...
extensions: [irori-tool-gate]                    # 显式确保围栏被加载
...
| P1 | 抽 `irori-tool-gate` 为路径扩展 + `writeToolGateConfig` 落盘 | 主会话行为零回归 | ✅ 已建成 |
...
- `pi-session-adapter.mjs`：新增 `resolvePiSubagentsPackageRoot`；`buildPiResourceLoaderOptions`
  支持 `additionalPackageRoots`；`createIroriPiSession` 增 opt-in `enableSubagents`
...
- **A（推荐）gate 打包 + 继承**：把 `irori-tool-gate` 做成扩展包加入父
...
- `pi-session-adapter.mjs`：`enableSubagents` 且有 `gatePolicy` 时，把
  pi-subagents 包根 + Irori gate 包根一起加入 `additionalExtensionPaths`，
...
2. **配置传递**：`IRORI_TOOL_GATE_CONFIG` 环境变量是否随子进程继承，使子进程读到正确 policy。
...
4. 解决 1-3 后，才能回到 B 的原始问题：**子进程是否继承 gate 包 + `IRORI_TOOL_GATE_CONFIG`**。
...
> 必须把 `irori-tool-gate` 包路径显式注入每个子 agent 的 `extensions` frontmatter，
...
- 新增 `apps/desktop/sidecar/src/tool-gate-config.mjs`：`buildToolGateConfig` /
  ...
  默认路径 `~/.pi/irori-tool-gate.json`，环境变量 `IRORI_TOOL_GATE_CONFIG`。
- 新增 `apps/desktop/sidecar/src/extensions/irori-tool-gate.mjs`：路径可加载扩展，
...
- 测试：`tool-gate-config.test.mjs`（5）、`irori-tool-gate.test.mjs`（6）、
