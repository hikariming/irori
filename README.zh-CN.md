...
|---|---|---|---|---|
| 核心定位 | 能真正干活的二次元伴侣 | 角色扮演与聊天前端 | 语音互动的虚拟主播陪伴 | 通用写码 / 自动化 agent |
| 原生桌面应用 | ✅ Tauri 外壳 | ⚠️ 本地 Web UI | ⚠️ Web / 桌面 | ⚠️ 终端 / CLI 为主 |
| 智能体协作（写作 · 写码 · 规划 · 跟进） | ✅ 针对代码与复杂任务优化 | ❌ 仅聊天 | ❌ 仅聊天 | ✅ 通用 agent |
| 二次元角色陪伴 | ✅ 核心体验 | ✅ | ✅ | ❌ |
| 角色卡 | ✅ 可编辑 schema + 提示词组合 | ✅ | ✅ | ❌ |
| 本地记忆与上下文 | ✅ 内置，本地优先 | ✅ Lorebook / 世界书 | ✅ | ✅ |
| 工具调用与安全门 | ✅ 确认策略 + 受保护路径 | ❌ | ❌ | ✅ 工具调用 |
| 语音 / Live2D 形象 | 🔜 规划中 | ⚠️ 需插件 | ✅ 核心功能 | ❌ |
| 多渠道接入（Discord 等） | 🔜 规划中 | ⚠️ 需插件 | ⚠️ | ⚠️ |
| 陪伴手机 App | 🔜 规划中 | ⚠️ 移动端浏览器 | ⚠️ 移动端浏览器 | ❌ |
| 最适合 | 帮你思考、写作、交付的搭档 | 沉浸式角色扮演 | 和桌面老婆聊天 | 无界面任务自动化 |

它们在各自的赛道上都很出色：SillyTavern 适合沉浸式角色扮演，Open-LLM-VTuber 适合会说话的 Live2D 形象，OpenClaw 适合无界面自动化。Irori 的定位是把**真正的二次元角色陪伴**和一个**针对代码与复杂任务优化**的 agent 结合在原生桌面应用里——多渠道接入和手机 App 也已在规划中。
